...
async function fetchMovies() {
    const tmdbRequest = Array(20).fill(null).map((_n,idx) => {
        return axios.get(`https://api.themoviedb.org/3/movie/popular?api_key=ebd02a02c91cc571a5143e6ec6241552&language=en-US&page=${idx+1}`);
    });
    ...
    const fromTMDB = movies.slice(0,-1).map(elem => {
        return elem.results;
    }).reduce((acum, item) => [...acum, ...item], []).map(elem => {
