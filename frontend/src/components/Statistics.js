    ...
    return (
        <div id="statsContent">
            <div>
                ...
                {roles ? roles.map((elem,idx) => <div key={elem.id}>
                    <p><strong>{idx+1}. </strong>{elem.name}: {elem.roles}</p>
                </div>) : null}
            </div>
            <div>
                <Title level={3}>Number of given comments: </Title>
                {commented ? commented.map((elem,idx) => <div key={elem.id}>
                    <p><strong>{idx+1}. </strong>{elem.login}: {elem.comments}</p>
                </div>) : null}
            </div>
            <div>
                <Title level={3}>Number of movies by genre: </Title>
                {genreStats ? genreStats.map((elem,idx) => <div key={elem.id}>
                    <p><strong>{idx+1}. </strong>{getGenreName(elem.genreId)}: {elem.amount}</p>
                </div>) : null}
            </div>
        </div>
    )
